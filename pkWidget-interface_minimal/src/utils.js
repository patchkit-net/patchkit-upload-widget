...
export const setUploadData = (data) => {
  ...
  totalChunks = Math.ceil(data.archive.size / config.chunkSize);
  return data;
};

export const chunkedFileUpload = (callbacks = {}, start = 0) => {
  if (!uploadData.url || !uploadData.archive || !uploadData.jwt || totalChunks <= 0) {
  ...
  const newChunk = _createChunk(uploadData.archive, start);
  let end = loadedChunks * config.chunkSize - 1;
  if (loadedChunks === totalChunks) {
    end = uploadData.archive.size - 1;
  }
  ...
  const chunkUploadError = () => {
    ...
    _uploadChunk(uploadData.archive.size, newChunk, start, end, { 
